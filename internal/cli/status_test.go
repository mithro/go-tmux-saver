...
// TestStatusCheckFreshExitCode covers the watch unit's contract: `status
// --check-fresh` exits non-zero when the newest good save is stale and zero
// once it is fresh. That non-zero exit is the entire journal/systemd signal
// go-tmux-saver-watch.service raises — no mail, no markers, no side effects.
func TestStatusCheckFreshExitCode(t *testing.T) {
	dataDir := t.TempDir()
	cfgPath := writeConfig(t, `{}`)

	// Stale (no good save at all): exit 1.
	var out, errb bytes.Buffer
	if code := Run([]string{"status", "--check-fresh", "--config", cfgPath, "--data-dir", dataDir}, &out, &errb); code != 1 {
		t.Fatalf("stale exit %d, want 1; stdout=%q stderr=%q", code, out.String(), errb.String())
	}
...
// previously untested) — the watch unit and humans both read it.
