...
// live ExecStartPost showing the drop-in's restore line.
...
func TestUpdateDryRunThenApplies(t *testing.T) {
	...
	if err := os.WriteFile(driftPath, []byte("garbage, not a rendered unit"), 0o644); err != nil {
		t.Fatalf("drift %s: %v", RelTimer, err)
	}
	fake.calls = nil // isolate the calls Update itself makes

	changed, err := Update(env, files, true)
	if err != nil {
	...
	if string(got) != "garbage, not a rendered unit" {
